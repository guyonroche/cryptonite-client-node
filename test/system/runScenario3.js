const runScenario3 = (trader2) => {
  ...
  return trader2.placeLimitOrder('S', trader2.getBalance().assets + 25, 0.14)
    .then(() => { throw new Error('The function resolved when it shouldn\'t have');},
    ...
    .then(() => trader2.placeLimitOrder('B', ((trader2.getBalance().capital / 0.12) + 25), 0.12))
    .then(() => { throw new Error('The function resolved when it shouldn\'t have');},
      (err) => { console.log(err.response.data.message);})
