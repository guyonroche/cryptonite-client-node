...
module.exports = class CryptoniteClient extends EventEmitter {
  constructor({ protocol = 'https:', hostname, apiKey, apiSecret, httpAgent, options = {} }) {
    super();
    this.protocol = protocol;
    this.hostname = hostname;
    this.apiKey = apiKey;
    this.apiSecret = apiSecret;
    this.httpsAgent = httpAgent;
    this.verbose = options.verbose;
    this.nonce = 0;

    this.axios = axios.create({
      httpsAgent: new https.Agent(this.httpAgent),
    });
  ...
  connect() {
    ...
    return new Promish((resolve, reject) => {
      let satisfied = false;
      try {
        this.ws = new WebSocket(wsUrl, this.agent);
